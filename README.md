...
**That's it!** Authentication uses Azure Entra ID:

- ✅ `az login` authenticates with your Azure account
- ✅ `./setup.sh` verifies Azure authentication → creates `env.sh`
- ✅ `source env.sh` exports environment variables to your shell
- ✅ MCP server uses `AzureCliCredential` to acquire tokens automatically
- ✅ GraphQL API validates JWT tokens with Microsoft.Identity.Web
- ✅ NO bearer tokens or secrets - uses Azure AD authentication
- ✅ User tracking: time entries include your Azure AD identity

**100% Azure AD authentication - secure and auditable!**
...
Note: env.sh (generated by setup.sh) contains secrets and is in .gitignore
```
...
- **Azure Entra ID Authentication** for API access with JWT token validation
- **User Identity Tracking** - All time entries include Azure AD user information
- **Input Validation** at multiple layers (MCP, GraphQL, Business Logic, Database)
- **Environment Variables** for configuration (no secrets stored)
- **AzureCliCredential** for local development, **ManagedIdentity** for production

...
public class WorkflowMutationTests : IClassFixture<PostgresContainerFixture>, IAsyncLifetime
{
    ...
    private static ClaimsPrincipal CreateTestUser(string email = "test@example.com", string name = "Test User", string oid = "test-oid-123")
    {
        var claims = new List<Claim>
        {
            ...
            new Claim("extension_8b3f87d7bc23493288b5f24056999600_TimeReportingACL", "Project=V,E,A,M,T")
        };
    ...
    public async Task SubmitTimeEntry_WithNotReportedStatus_ChangesStatusToSubmitted()
    {
        ...
        // Act
        var result = await _mutation.SubmitTimeEntry(entry.Id, _testUser, _context);
    ...
    public async Task ApproveTimeEntry_WithSubmittedStatus_ChangesStatusToApproved()
    {
        ...
        // Act
        var result = await _mutation.ApproveTimeEntry(entry.Id, _testUser, _context);
    ...
    public async Task DeclineTimeEntry_WithSubmittedStatus_ChangesStatusToDeclined()
    {
        ...
        var result = await _mutation.DeclineTimeEntry(entry.Id, comment, _testUser, _context);
